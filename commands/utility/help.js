...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    try {
      const Categories = ['admin', 'fun', 'images', 'info', 'mod', 'utility']
      const AllCommands = []
      ...
      for (let i = 0; i < Categories.length; i++) {
        const Cmds = await bot.commands
          .filter((C) => C.config.category === Categories[i])
          .array()
          .map((C) => C.config.name)
          .sort((a, b) => (a < b ? -1 : 1))
          .join(', ')
      ...
      const Embed = new Discord.MessageEmbed()
        ...
        .setDescription(
          Description +
            ...
            '\n\n' +
            '**Links -**' +
            ` [Join Support](${support}) • [Invite Me](https://discordapp.com/oauth2/authorize?client_id=${bot.user.id}&permissions=8&scope=bot)`
        )
        .setFooter(
    ...
    ** Examples -** \`${
      command.example ? `${command.example}` : 'No Examples Found'
    }\`\n
    ** Aliases -** [ \`${
