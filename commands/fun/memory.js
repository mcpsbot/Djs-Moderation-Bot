...
module.exports = {
  config: {
    ...
    aliases: [""],
    category: "fun",
    usage: "memory [number](1 - 20)",
    description: "Test Your Memory",
  },
  run: async (bot, message, args, ops) => {
    if (!args[0]) {
      return message.channel.send("**Please specify a level between 1-20**");
    }
    const level = args[0];
    if (level < 1 || level > 20) {
    ...
    try {
      ...
      const messages = await message.channel.awaitMessages(
        (res) => message.author.id === res.author.id,
        {
          max: 1,
          time: 30000,
        }
      ...
      if (!messages.size) {
        return message.channel.send(`**Time Uup! It Was ${memorizeDisplay}!**`);
      }
      const answer = messages.first().content.toLowerCase();
      if (answer !== memorizeType) {
        return message.channel.send(
          `**You Typed It Wrong, It Was ${memorizeDisplay}!**`
        );
      }
