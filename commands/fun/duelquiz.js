  ...
  run: async (bot, message, args, ops) => {
    ...
    if (!opponent) {
      return message.channel.send(
        "**Please Enter A Valid User! Ex- duelquiz <@user> <required points 1-8>**"
      );
    }
    const maxPts = args[1];
    if (maxPts <= 0 || maxPts >= 9) {
      return message.channel.send("**Please Enter A Number Between 1-8!**");
    }
    ...
    try {
      ...
      while (!winner) {
        ...
        const filter = (res) => {
          ...
          if (!choices.includes(choice) || answered.includes(res.author.id)) {
            return false;
          }
          if (![message.author.id, opponent.id].includes(res.author.id)) {
            return false;
          }
          answered.push(res.author.id);
          if (
            question.answers[choices.indexOf(res.content.toUpperCase())] !==
            question.correct
          ) {
            message.channel
              .send(`**${res.author}, That's Incorrect!**`)
              .catch(() => null);
            return false;
          }
          return true;
