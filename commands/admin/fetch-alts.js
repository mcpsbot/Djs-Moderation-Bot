...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    const days = args[0];
    if (!days) {
      return message.channel.send("Please provide a valid days duration");
    }
    ...
    if (day > 100) {
      return message.channel.send(
        "You may only find alts of an account age of **100 days** or below"
      );
    }
    ...
    message.guild.members.cache.forEach(async (user) => {
      const math = day * 86400000;

      const x = Date.now() - user.user.createdAt;
