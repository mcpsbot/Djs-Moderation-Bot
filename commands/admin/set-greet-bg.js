        ...
        msg.channel
          ...
          .then((collected) => {
            switch (collected.first().content) {
                      ...
                      switch (collected.first().content.toString()) {
                        ...
                        default:
                          if (collected.first().attachments.size > 0) {
                            if (
                              collected.first().attachments.every(attachIsImage)
                            ) {
                              message.channel.send(
                                new MessageEmbed()
                                  ...
                                  .setColor("RANDOM")
                                  .setDescription(
                                    "I have successfully seted the welcome image in this server.\n\nPlease make sure to **not** delete your Image from the Channel!"
                                  )
                                  .setFooter(
                              ...
                              message.reply(
                                "Could not set your message as a background image"
                              );
                            }
                          } else if (
                            ...
                            message.reply(
                              "Successfully, set your Background Image! Please make sure to **not** delete your Image from the Channel!"
                            );

                            db.set(
                              `WelIm_${message.guild.id}`,
                              collected.first().content
                            );
                          } else {
                            message.reply(
                              "Could not your message as a background image"
                            );
                          }
                          break;
                      }
              ...
              case "2":
                const vcheck = db.fetch(`WelIm_${message.guild.id}`);
                if (vcheck === true) {
                  ...
                  return message.channel.send(
                    new MessageEmbed()
                      ...
                      .setColor("RANDOM")
                      .setDescription(
                        "I have successfully deleted the Background Image in the server"
                      )
                      .setTimestamp()
                  );
                } else {
        ...
        msg.channel
          ...
          .then((collected) => {
            switch (collected.first().content) {
                ...
                message.reply(cembed).then((msg) => {
                  msg.channel
                    ...
                    .then((collected) => {
                      switch (collected.first().content.toString()) {
                        case "1":
                          const check = db.fetch(`Leaveim_${message.guild.id}`);
                          if (check === null) {
                            if (collected.first().attachments.size > 0) {
                              if (
                                collected
                            ...
                            message.channel.send(
                              "There's already a Welcome Image seted in the server.If you wanna add a new, Please disable it and then run the setup again!!"
                            );
                          }
                          break;
                      }
              ...
              case "2":
                const vcheck = db.fetch(`Leaveim_${message.guild.id}`);
                if (vcheck === true) {
