...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    const leave = ["leave", "left"];
    const Types = [];
    Welcome.forEach((wel) => Types.push(wel));
    leave.forEach((leav) => Types.push(leav));

    if (!Type || !Types.find((T) => T === Type.toLowerCase())) {
      return message.channel.send(
        "Please Give A Valid Type - Welcome, Wel, Join, Leave, Left"
      );
    }
    ...
    const Current = await GetType(Type);
    ...
    const Embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        `${
          Current === "Welcome" ? "Welcome" : "Leave"
        } Channel Has Been Setted - <#${Channel.id}>`
      )
      .setFooter(`Setted By ${message.author.username}`)
      .setTimestamp();

    await db.set(
      `${Current === "Welcome" ? "Welcome" : "Leave"}_${
        message.guild.id
      }_Channel`,
      Channel.id
    );
    ...
    try {
      ...
      return message.channel.send(
        `${
          Current === "Welcome" ? "Welcome" : "Leave"
        } Message Has Been Setted - <#${Channel}>`
      );
    }
  },
};
