...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    const option = args[0];
    const options = ["punishment", "age", "logs", "show", "disable"];
    if (!option) {
      return message.channel.send(
    ...
    switch (option.toLowerCase()) {
      ...
      case "age":
        ...
        if (!days) {
          return message.channel.send(
            "Please specify a valid day, it also must be a number. Ex- alt-config age 7"
          );
        }
        if (isNaN(days)) {
          return message.channel.send(
            "Please specify a valid day, it also must be a number"
          );
        }
        ...
        if (day > 100) {
          return message.channel.send(
            "Please donot exceed the length of 100 days.."
          );
        }
      ...
      case "show":
        ...
        const ageee = db.get(`age.${message.guild.id}`) || 0;
      ...
      case "disable":
        check =
          ...
          db.get(`punishment.${message.guild.id}`) &&
          db.get(`age.${message.guild.id}`);

        if (!check) {
          return message.channel.send(
            "Please set the required fields first or i cant disable it!!"
          );
        } else {
          ...
          return message.channel.send(
            "Disabled the Anti-Alt logging in the server :)"
          );
        }
    }
