  ...
  run: async (bot, message, args) => {
    if (
      ...
      return message.channel.send(
        "**You Dont Have The Permissions To Ban Users! - [MOVE_MEMBERS]**"
      );
    }
    ...
    const channel =
      ...
      bot.guilds.cache.get(message.guild.id).channels.cache.get(args[1]) ||
      message.guild.channels.cache.find(
        (c) =>
          c.name.toLowerCase() === args.slice(1).join(" ").toLocaleLowerCase()
      );
