...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    const warnings = await db.fetch(
      `warnings_${message.guild.id}_${member.id}`
    );

    if (warnings === 3) {
      return message.channel.send(
        `${member} already reached his/her limit with 3 warnings`
      );
    }
    ...
    if (warnings === null) {
      ...
      member
        .send(
          `You have been warned by <${message.author.username}> for this reason: ${reason}`
        )
        .catch((error) =>
          message.channel.send(
            `Sorry <${message.author}> I couldn't n't warn because of : ${error}`
          )
        );
      const warnEmbed = new MessageEmbed()
        .setTitle("**__Warn Report__**")
        .setDescription(
          `**<@${member.user.id}> has been warned by <@${message.author.id}>**`
        )
        .addField("**Reason:**", `\`${reason}\``)
        .addField("**Action:**", "`Warn`")
        .addField("**Moderator:**", `${message.author}`);

      message.channel.send(warnEmbed).then((msg) =>
        msg.delete({
          timeout: 5000,
        })
      );
