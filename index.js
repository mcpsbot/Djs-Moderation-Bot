...
bot.on("ready", () => {
  ...
  console.log(
    `[INFO]: watching ${bot.guilds.cache.size} Servers, ${bot.channels.cache.size} channels & ${bot.users.cache.size} users`
  );
  console.log("-------------------------------------");
...
bot.on("message", async (message) => {
  ...
  if (message.content.match(mentionRegex)) {
    message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `Hey <@${message.author.id}>, My prefix for this guild is \`\`\`${Prefix}\`\`\`.Use \`\`\`${Prefix}help\`\`\` or <@${bot.user.id}> help to get a list of commands`
        )
        .setColor("RANDOM")
  ...
  const cmdx = wb.fetch(`cmd_${message.guild.id}`);
...
bot.on("message", async (message) => {
  ...
  const chatbot = bot.setups.get(message.guild.id, "aichatsystem");
...
bot.on("guildMemberAdd", async (member) => {
  ...
  const roles = bot.setups.get(member.guild.id, "welcome.roles");

  if (roles.length >= 1) {
    for (let i = 0; i < roles.length; i++) {
      try {
        const roleadd = member.guild.roles.cache.get(roles[i]);
  ...
  const toggle = await db.fetch(`Weltog_${member.guild.id}`);
  const togEm = await db.fetch(`Welemtog_${member.guild.id}`);
  ...
  if (toggle === true) {
    if (togEm === true) {
      try {
        const sChannel = await db.fetch(`Welcome_${member.guild.id}_Channel`);
        if (!sChannel) return;
        let sMessage = await db.fetch(`Welcome_${member.guild.id}_Msg`);
        if (!sMessage) sMessage = "Welcome To The Server!";
        const sWelcomeImage = await db.fetch(`WelIm_${member.guild.id}`);
        ...
        const sWelcomed = new canvas.Welcome();
        const sImage = await sWelcomed
      ...
      try {
        const Channel = await db.fetch(`Welcome_${member.guild.id}_Channel`);
        if (!Channel) return;
        let Message = await db.fetch(`Welcome_${member.guild.id}_Msg`);
        if (!Message) Message = "Welcome To The Server!";
        const WelcomeImage = await db.fetch(`WelIm_${member.guild.id}`);
        ...
        const Msg = Message.replace(/{user}/g, `${member}`)
          .replace(/{user_tag}/g, `${member.user.tag}`)
          .replace(/{user_name}/g, `${member.user.username}`)
          .replace(/{user_id}/g, `${member.id}`)
          .replace(/{server_name}/g, `${member.guild.name}`)
        ...
        const Welcomed = new canvas.Welcome();
        const Image = await Welcomed.setUsername(member.user.username)
          .setDiscriminator(member.user.discriminator)
          .setGuildName(member.guild.name)
          .setAvatar(
            member.user.displayAvatarURL({ dynamic: false, format: "jpg" })
...
bot.on("guildMemberRemove", async (member) => {
  const toggle = await db.fetch(`leavtog_${member.guild.id}`);
  const togEm = await db.fetch(`leavemtog_${member.guild.id}`);

  if (toggle === true) {
    if (togEm === true) {
      try {
        const sChannel = await db.fetch(`Leave_${member.guild.id}_Channel`);
        if (!sChannel) return;
        let sMessage = await db.fetch(`Leave_${member.guild.id}_Msg`);
        if (!sMessage) {
          sMessage = `${member.user.username} Has Left The Server!`;
        }
        const sLeaveImage = await db.fetch(`Leaveim_${member.guild.id}`);
        ...
        const sLeaved = new canvas.Goodbye();
        const sImage = await sLeaved
          .setUsername(member.user.username)
          .setDiscriminator(member.user.discriminator)
          .setGuildName(member.guild.name)
          .setAvatar(
      ...
      try {
        const Channel = await db.fetch(`Leave_${member.guild.id}_Channel`);
        if (!Channel) return;
        let Message = await db.fetch(`Leave_${member.guild.id}_Msg`);
        if (!Message) Message = `${member.user.username} Has Left The Server!`;
        const LeaveImage = await db.fetch(`Leaveim_${member.guild.id}`);
        ...
        const Leaved = new canvas.Goodbye();
        const Image = await Leaved.setUsername(member.user.username)
          .setDiscriminator(member.user.discriminator)
          .setGuildName(member.guild.name)
          .setAvatar(
            member.user.displayAvatarURL({ dynamic: false, format: "jpg" })
...
bot.on("guildMemberAdd", async (member) => {
  if (!member.guild) return;
  const age = await wb.get(`age.${member.guild.id}`);
  const logs = await wb.get(`logs.${member.guild.id}`);
  const punishment = wb.get(`punishment.${member.guild.id}`);
  const bypassed = await wb.get(`bypass.${member.guild.id}`);
  if (!bypassed.includes(member.id)) {
    ...
    if (day >= created) {
      ...
      const channel = await bot.channels.cache.get(logs);
